...
s.listed_names={CARD_RA,513000134}
s.listed_series={SET_SUN_GOD,SET_THE_WINGED_DRAGON_OF_RA}

function s.gybanspfilter(c,e,tp)
	return (c:IsSetCard(SET_THE_WINGED_DRAGON_OF_RA) or c:IsCode(CARD_RA) or c:IsCode(513000134)) 
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if sc and Duel.SpecialSummonStep(sc,0,tp,tp,true,false,POS_FACEUP) then
		local isMainRa = (sc:IsCode(CARD_RA) or sc:IsCode(513000134)) and not sc:IsCode(100000010)
		
		if isMainRa then
			Duel.SpecialSummonComplete()
			-- Comprobamos si el jugador quiere hacer la transición y si hay espacio en el Extra Deck
			if sc:IsAbleToGrave() and Duel.IsExistingMatchingCard(s.extraspfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,sc)
				and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
				Duel.BreakEffect()
				
				-- ENVIAR COMO COSTE (REASON_COST): Esto ignora las inmunidades del Ra del anime con éxito
				if Duel.SendtoGrave(sc, REASON_COST|REASON_EFFECT)>0 and sc:IsLocation(LOCATION_GRAVE) then
					sc=nil
					Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
					local sc2=Duel.SelectMatchingCard(tp,s.extraspfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp):GetFirst()
					if sc2 then
						Duel.SpecialSummonStep(sc2,0,tp,tp,false,false,POS_FACEUP)
						sc = sc2 -- Asignamos el nuevo monstruo para que la End Phase lo afecte si es necesario
					end
				end
			end
		else
	...
	if sc then
		-- Durante la End Phase, envía el monstruo al Cementerio
